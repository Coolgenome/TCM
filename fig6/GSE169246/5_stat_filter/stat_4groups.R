...
for(tt in targetTreatmentType){
    for(ami in 1:length(allMD)){
        ...
        pdf(file.path(figurePath, paste0("TreatmentType_", tt, "_", tempName, "_4group_response_roe_heatmap.pdf")), width = widthi, height = heighti)
        print(pheatmap(matRoe,
                       ...
                       border_color = F))
        dev.off()
        pdf(file.path(figurePath, paste0("TreatmentType_", tt, "_", tempName, "_4group_response_cell_heatmap.pdf")), width = widthi * 1.5, height = heighti)
