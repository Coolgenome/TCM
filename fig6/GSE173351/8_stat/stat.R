...
CD4_meta <- CD4PredictedObj@meta.data %>% filter(tissue == "tumor")
CD4_meta$isResponse = "No"
CD4_meta$isResponse[CD4_meta$response %in% "MPR"] = "Yes"

CD8_meta <- CD8PredictedObj@meta.data %>% filter(tissue == "tumor")
CD8_meta$isResponse = "No"
CD8_meta$isResponse[CD8_meta$response %in% "MPR"] = "Yes"
...
for(tCS in targetCellSet){
    for(tT in targetTissues){
        for(ami in 1:length(allMD)){
            ...
            figurePath <- file.path("/rsrch3/scratch/genomic_med/ychu2/projects/p1review/R3Q7/result/GSE173351/7_mapping_stat",
                                    tempName)
