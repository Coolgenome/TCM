...
allMD <- list(CD8 = CD8_meta, CD4 = CD4_meta, Proliferative = P_meta)
...
for (tCS in targetCellSet) {
    for (tT in targetTissues) {
        for (ami in 1:length(allMD)) {
            ...
            ## ggsave(file.path(figure_path, paste0(tempName, "-", tCS, "_Tissue-", tT,"_response_bar.pdf")), g, width = 200, height = 1200, units = "mm")
            ...
            pdf(file.path(figure_path, paste0(tempName, "-", tCS, "_Tissue-", tT, "_response_roe_heatmap.pdf")), width = widthi, height = heighti)
            print(pheatmap(matRoe,
                           ...
                           border_color = F))
            dev.off()
            pdf(file.path(figure_path, paste0(tempName, "-", tCS, "_Tissue-", tT, "_response_cell_heatmap.pdf")), width = widthi * 1.5, height = heighti)
    ...
    for(tempGroup in unique(tempMD$group)) {
        ...
        for(gsci in unique(tempMD$predicted.id)) {
            ...
            tempData <- tibble(group = tempGroup,
                               Cluster = gsci,
                               Roe = (n/M) / (k/N))
            tempRowName <- paste0(tempName, "_c", as.character(gsci))
            tempColName <- tempGroup
            matRoe[tempRowName, tempColName] <- (n/M) / (k/N)
            matCell[tempRowName, tempColName] <- n
        }
...
colnames(matRoe) <- paste0("173351", "-", colnames(matRoe))
colnames(matCell) <- paste0("173351", "-", colnames(matCell))

totalMatRoe <- cbind(totalMatRoe, matRoe)
totalMatCell <- cbind(totalMatCell, matCell)

colnames(totalMatRoe) <- paste0("NSCLC_", colnames(totalMatRoe))
colnames(totalMatCell) <- paste0("NSCLC_", colnames(totalMatCell))

saveRDS(totalMatRoe, file.path(figurePath, paste0('NSCLC_matRoe.rds')))
saveRDS(totalMatCell, file.path(figurePath, paste0('NSCLC_matCell.rds')))
